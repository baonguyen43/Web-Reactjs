...
class Type15_Mix extends Component {
  constructor(props) {
    super(props);
    this.state = {
      question: [],
  ...
  checkAnswer = async () => {
    const { cards, arrayText, arrow } = this.state;
    let score = 0;
    ...
    score > 50 ? openNotificationWithIcon('success', 'CORRECT') : openNotificationWithIcon('danger', 'INCORRECT');
  ...
  postAnswerToAPI = (score) => {
    ...
    const { questionContent, arrayText, cards } = this.state;
    ...
    return arrayText.map((item, index) => {
      return (
        <div key={index} style={{ marginTop: 10 }}>
          <Button style={{ height: 150, width: 190 }}>{item.Text}</Button>
        </div>
      );
    });
  };
