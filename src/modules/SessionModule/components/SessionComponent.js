import { Avatar, BackTop, Rate } from 'antd';
...
class SessionComponent extends React.Component {
  ...
  getSessions = async () => {
    ...
    const studentId = typeClass === 'AMES' ? loggedInUser.userMyames.StudentId : loggedInUser.userMyai.StudentId;

    this.setState({ studentId }); // Lấy studentId dùng cho isMix4.
  ...
  renderItem = () => {
    ...
    const imageUrl = require('assets/img/New_English_bg.png');
      ...
      return (
        <Col xs={12} sm={6} md={4} lg={4} xl={4} xxl={4} key={index}>
          ...
          <Card style={{ cursor: 'pointer' }} onClick={() => this.moveToNextPage(item)}>
            ...
            <CardBody>
              <CardTitle className="mb-3 text-info" style={{ fontWeight: '700', fontSize: 15 }}>
                <Avatar
                  style={{
              ...
              <div className="progress-wrapper">
                ...
                <Progress max="100" value={item.completedPercent} color="info" />
              </div>
  ...
  moveToNextPage = (item) => {
    ...
    const { history, dispatch, match, isMix4 } = this.props;
    const classID = classId || match?.params?.classId;
    ...
    if (item.enableAddStar) {
      ...
      if (isMix4) {
        return history.push({
          ...
          pathname: `/ames/class/${classID}/session/${item.id}/assignmentsMix4`,
        });
      }
      return history.push({
        pathname: `/ames/class/${classID}/session/${item.id}/questionsMix`,
        // search: `?type=${item.questionType}&asrId=${item.asrId}&takeExamTime=${this.takeExamTime}&questionId=${item.questionId}`,
      });
    }

    return history.push({
      pathname: `/ames/class/${classID}/session/${item.id || item.sessionId || item.level}/assignments`,
      // search: `?type=${item.questionType}&asrId=${item.asrId}&takeExamTime=${this.takeExamTime}&questionId=${item.questionId}`,
    });
  };
  // moveToNextPage = (item) => {
...
const mapStateToProps = (state) => {
  return {
    ...
    isMix4: state.sessionReducer.isMix4,
  };
};

const mapDispatchToProps = (dispatch) => ({
  fetchSessions: (payload) => dispatch({ type: ActionTypes.FETCH_SESSION, payload }),
  dispatch,
});
