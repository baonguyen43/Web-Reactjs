...
const Type20Modal_Mix = ({ detail, type }) => {

  if (!detail) return null;
  const studentChoices = JSON.parse(detail.studentChoices);
  const grammarQuestion = JSON.parse(detail.grammarQuestion);
  ...
  return (
    <div className="modal-body text-center">
      <ListGroup>
        ...
        {/*  Trắc nghiệm */}
        <ListGroupItem>
          <p className='mt-2' style={{ fontSize: 18, fontWeight: '500' }}>{grammarQuestion[0].QuestionTextFormat}</p>
        ...
        <Listen
              custom
              audioURL={studentChoices.correctAudioUrl}
              className={'question-type__audioExampleType02 ml-2'}
            >
              <i className="fas fa-volume-up"></i>
            </Listen>
...
Type20Modal_Mix.propTypes = {
  detail: PropTypes.instanceOf(Object),
  type: PropTypes.string
}
export default Type20Modal_Mix