...
const Select3 = ({ question, audio }) => {
  ...
  const postAnswer = useCallback((answers, sentences, state) => {
    ...
    // Gởi
    functions.postAnswerToAPI(result).then(response => console.log('Send S3 answers: success')).catch(error => console.log('Send S3 answers', error))
    // Cập nhật điểm.
  ...
  const onFinish = React.useCallback(
    (value) => {
      const arrayHint = ReactHtmlParser(state.questions[0].hintBox)?.[0]?.props?.children[0].split('|')
      const { questions } = state;
      let checkAnswerArray = [];
      questions[0].answers.forEach((answer, indexAnswer) => {
        const isCorrect =
          answer.text === arrayHint[value[indexAnswer]]?.trim()
      ...
      questions[0].checkAnswerArray.map((item, index) => {
        ...
        return answers.push({ indexKeyWord: !isNaN(indexKeyWord) ? indexKeyWord : -1, answer: answer ?? '', isCorrect: item })
      })
  ...
  const contentSelect = React.useCallback(() => {
    let temp = [];
    const arrayHint = ReactHtmlParser(state.questions[0].hintBox)?.[0]?.props?.children[0].split('|')
    return arrayHint?.map((item, index) => {
        ...
        return (
          <span key={index} style={{
            ...
            {elementArray.map((item, index) => {
              ...
              if (state.isPointed) {
                isCorrect =
                  state.questions?.[0].checkAnswerArray?.[currentInputNo];
  ...
  const renderKeyWord = React.useCallback((item) => {
    let temp = [];
    const arrayHint = ReactHtmlParser(item.hintBox)?.[0]?.props?.children[0].split('|')
    return arrayHint?.map((itemWord, index) => {
      // kiểm tra đã có giá trị trong 'temp' chưa? Nếu chưa có thì thêm giá trị, tránh trùng lặp
      if (!temp.includes(itemWord.toLowerCase().trim())) {
        temp.push(itemWord.toLowerCase().trim())
        return (
          <span
            style={{
            ...
            key={index}
          >
            <span style={{ color: specifications.TEXT_WHITE, fontWeight: 400 }}>
              {itemWord.trim()}
            </span>
          </span>
        );
      ...
      return (
        <React.Fragment key={index}>
          ...
          <Row style={{ padding: 0, margin: 0 }}>
            ...
            >
              <span style={{ fontWeight: specifications.QUESTION_FONT_WEIGHT, lineHeight: specifications.Line_height }}>{ReactHtmlParser(item.title, { transform })}</span>
