...
const M3 = ({ question, audio }) => {
  ...
  const postAnswer = useCallback((answers, sentences, state) => {
    ...
    // Gởi
    functions.postAnswerToAPI(result).then(response => console.log('Send M3 answers: success')).catch(error => console.log('Send M3 answers', error))
    // Cập nhật điểm.
  ...
  const onSubmit = React.useCallback(() => {
    ...
    //   const leftInResult = resultsArray.find((x) => parseInt(x.left) === item.id);
  ...
  return (
    <React.Fragment>
      <CardBody style={{ overflowY: 'auto', overflowX: 'hidden', padding: 0 }}>
        <Row>
          <Col>
            <Row className="container-fluid" align='middle'>
              <Col>
                <div ref={dropLeft}>
                  {cardsLeft.map((card) => (<Card key={card.id} id={`${card.id}`} text={card.text} moveCard={moveCardLeft} findCard={findCardLeft} height={heightRef.current} image={card.image} />))}
                </div>
              </Col>
              <Col span={4}>
                {state.arrayArrow.map((item, index) => {
                  return (
                    <Row key={index} className='flex-1' style={{ minHeight: 60, alignItems: 'center', justifyContent: 'center', fontSize: 28, marginTop: 6, color: state.isPointed ? item.isCorrect ? '#27ae60' : '#e74c3c' : 'black' }}>
